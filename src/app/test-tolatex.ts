import {isNumber, insertCommas, ToLatex} from './tolatex'
  ...
  describe('Helper functions:', function() {
    describe('isNumber(n)', function() {
      it('should return false if n contains any characters besides numbers, periods, and minus signs', function() {
        assert.equal(false, isNumber('123f31'))
        assert.equal(false, isNumber('123-+kdsjfls.,ds'))
        assert.equal(false, isNumber('+123'))
      })
      ...
      it('should return true if n is a float', function () {
        assert(isNumber('123.455'))
        assert(isNumber('-123.999'))
        assert(isNumber('.123'))
        assert(isNumber('10.'))
      })
    ...
    describe('insertCommas(n)', function() {
      it('should not insert a comma if n has three or less non-decimal digits', function () {
        assert.equal('123', insertCommas('123'))
        assert.equal('999', insertCommas('999'))
        assert.equal('531.123431', insertCommas('531.123431'))
      })

      it('should insert commas every three non-decimal digits', function () {
        assert.equal('123,456,789', insertCommas('123456789'))
        assert.equal('987,654.321123', insertCommas('987654.321123'))
      })

      it('should not insert commas in decimal places', function () {
        assert.equal('0.123456', insertCommas('0.123456'))
      })

      it('should not interpret negative signs as a digit', function () {
        assert.equal('-123,456', insertCommas('-123456'))
        assert.equal('-999', insertCommas('-999'))
      })
    })
  })
